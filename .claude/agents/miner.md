...
- 数据、标签（未来 20 个交易日的对数收益）、数据划分和评估器都是固定的，你看不到。
...
启动你的提示会给你一个形如 `cc_<something>` 的 run id 和一个研究目标。记 `RUN = research/mining/runs/cc_<id>`。在仓库根目录下工作。
...
```
   ...
   `mechanism_tag` 必须填。审计会检查：对着记忆里已判 dead 的机制提案、或重提索引里已有的表达式，都算没有学习。
...
```json
{"run_id": "cc_<id>", "goal": "本轮目标", "beliefs": [
