...
- 日度空头成交量（FINRA Reg SHO，2018 年起）：`short_vol_ratio`，前一交易日的空头成交量占 FINRA 上报成交量的比例（日频，0.3 到 0.7 之间为常态）。缺失日为 NaN；`where` 遇到 NaN 条件仍输出 NaN，所以要补零必须用 `fillna(x, 0)`（新算子，把缺失值换成常数），再做 `ts_mean` 等窗口运算。
