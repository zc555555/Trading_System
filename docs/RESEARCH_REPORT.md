...
Three readings. Orthogonality is not the problem: pairwise correlations of the per-date z-scored factor scores never exceed 0.20, and high52's residual IC — after cross-sectional regression on every other factor — is still positive in both segments, so it carries information the rest of the blend does not. Concentration is: on this ruler the book rests on high52, the only factor with a positive dev IC, monotonic deciles and a spread that survives costs; the top-weighted volatility factor (28.8% of the blend) is dev-negative with *inverted* deciles and owes its weight to a strong holdout — the weighting scheme rewards a recent run, which is exactly what the November review must adjudicate. And turnover is the silent tax: 70–84% of the top decile changes every 20-day hold, so at 15 bp round trip most decile spreads are consumed before they reach the book. Every factor fails the family-wise gate at |t| ≥ 3.02 (N = 20); how that gate should treat literature-backed factors versus data-mined ones was settled on 2026-09-02, before any agent-mined candidate existed: rulebook v2 (`evaluation/RULEBOOK.md`) keeps this Šidák bar for literature-backed hypotheses (track A) and gives data-mined candidates their own family under Benjamini–Hochberg FDR q = 0.10 on a one-sided holdout test in a direction declared before evaluation, with the same negative-tier veto (track B).

The card's obvious follow-up — *concentrate on high52* — was pre-registered and **rejected**. A high52-only book through the identical construction grid beats the blend in dev on both universes (0.37–0.46 vs 0.22–0.36 on the standard panel; 0.25–0.30 vs 0.05–0.14 on the 501-name panel) and then turns *negative* in holdout on the standard panel (−0.18 to −0.63 against the blend's 0.61–0.85). The mechanism is the same table read the other way: the blend's holdout is carried by volatility, its dev by high52. Two factors that lead in different regimes are not one weak factor and one lucky one — the blend's diversification across them is doing real work, and re-weighting on dev IC alone would remove exactly the factor that carried the holdout. On the 501-name panel the high52-only book is the most balanced configuration seen (dev 0.25–0.30, holdout 0.87–1.09); one holdout window, logged for November, not acted on.
