...
def run_walk_forward(cfg: WalkForwardConfig, max_folds: int | None = None,
    ...
    t0 = time.time()
    print("=" * 78)
    print("PURGED WALK-FORWARD EVALUATION")
    print("=" * 78)
    print(f"config: {asdict(cfg)}")

    df = pd.read_parquet(data_path)
    df = df.sort_values(['date', 'symbol']).reset_index(drop=True)
    labeled = df[df['future_return'].notna()]
    ...
    for k, (train_dates, test_dates) in enumerate(folds, 1):
        ...
        panel = predict_panel(fold, test_df)
        panel['fold'] = k
        panels.append(panel)
    ...
    print("\n" + "=" * 78)
    print(f"OUT-OF-SAMPLE DAILY RANK IC  "
          f"(dev < {HOLDOUT_START} <= holdout)")
    print("=" * 78)
    ...
    for col in pred_cols:
        ...
        for seg_name, seg in (('dev', panel[~holdout_mask]),
                              ...
                              ('all', panel)):
            s = summarize_ic(daily_rank_ic(seg, col))
    ...
    panel.to_parquet(RESULTS_DIR / "oos_predictions.parquet", index=False)
    with open(RESULTS_DIR / "walk_forward_report.json", "w") as f:
        json.dump(report, f, indent=2, default=float)
    pd.DataFrame(weight_rows).to_csv(RESULTS_DIR / "fold_weights.csv", index=False)

    print(f"\nsaved: {RESULTS_DIR / 'oos_predictions.parquet'}")
    print(f"saved: {RESULTS_DIR / 'walk_forward_report.json'}")
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    run_walk_forward(WalkForwardConfig(), max_folds=args.max_folds)
