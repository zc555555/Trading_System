...
DEV_COLUMNS = ["date", "candidate_id", "source", "expected_direction", "canonical",
               ...
               "cluster_rep", "redundant_with", "residual_dev_t"]
...
def load_beliefs(runs_dir: Path = RUNS) -> tuple[dict, list[str]]:
    ...
    mechanism. Returns (mechanism -> belief dict with run_id, ordered run ids)."""
...
def find_duplicate(ledger_path: Path, canonical: str) -> dict | None:
...
def full_stage_outcomes(ledger_path: Path) -> dict[str, dict]:
    ...
    if led.empty:
        return {}
    keep = [c for c in ("candidate_id", "stage", "mechanism_tag", "canonical") if c in led.columns]
    led = led[keep]
    ...
    for _, r in led[led["stage"].astype(str) == "full"].iterrows():
        ...
        out[cid] = {"adopted": cid in adopted,
...
# --------------------------------------------------------------------------
def build_memory(ledger_path: Path, runs_dir: Path = RUNS, max_chars: int = MAX_CHARS) -> str:
    beliefs, runs = load_beliefs(runs_dir)
    idx = expression_index(ledger_path)
    outcomes = full_stage_outcomes(ledger_path)
    not_adopted_tags = {v["mechanism_tag"].lower() for v in outcomes.values()
    ...
    if outcomes:
        ...
        parts.append("| 候选 | 机制 | 结果 |")
        parts.append("|---|---|---|")
        for cid, v in outcomes.items():
            parts.append(f"| {cid} | {v['mechanism_tag'] or '（无标签）'} | {'已采纳' if v['adopted'] else '未采纳'} |")
        if not_adopted_tags:
    ...
    if beliefs:
        ...
        parts.append("| 机制 | 状态 | 完整阶段 | 证据 | 下一步 | 轮次 |")
        parts.append("|---|---|---|---|---|---|")
        for b in rows:
            fs = "未采纳" if b["mechanism"].lower() in not_adopted_tags else ""
            parts.append(f"| {b['mechanism']} | {b['status']} | {fs} | {b['evidence'][:160]} | {b['next'][:160]} | {b['run_id']} |")
    else:
        parts.append("（尚无 beliefs 记录）")

    parts.append("\n## 已筛选表达式索引（重复提交会直接返回旧结果，不再计算）")
    if len(idx):
        parts.append("| canonical | 方向 | dev_ic | dev_t | 过筛 | 冗余于 | 残差t | 轮次 |")
        parts.append("|---|---|---|---|---|---|---|---|")
        idx2 = idx.copy()
        ...
        for _, r in idx2.sort_values("abs_t", ascending=False).iterrows():
            ...
            parts.append(f"| `{r['canonical']}` | {r['expected_direction']} | {ic} | {t} | {sp} | {red} | {rt} | {run} |")
    else:
        parts.append("（尚无筛选记录）")
...
def audit_run(run_dir: Path, ledger_path: Path, runs_dir: Path = RUNS) -> dict:
    ...
    for d in _run_dirs(runs_dir):
        ...
        if not f.exists():
            continue
        try:
            for b in json.loads(f.read_text(encoding="utf-8")).get("beliefs", []):
