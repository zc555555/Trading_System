"""
Train LightGBM Regression Model with SELECTED FEATURES (60 features).

Expected improvement: 57.95% -> 59-60% direction accuracy
"""
...
def train_lightgbm_selected():
    """Train LightGBM with selected 60 features."""

    print("=" * 80)
    print("TRAINING LIGHTGBM REGRESSION MODEL (SELECTED 60 FEATURES)")
    print("=" * 80)
    ...
    for key, value in params.items():
        if key not in ['verbosity', 'n_jobs', 'objective', 'metric', 'boosting_type']:
            print(f"  {key}: {value}")
    ...
    for idx, row in importance_df.head(15).iterrows():
        print(f"{row.name + 1:2d}. {row['feature']:30s} : {row['importance']:.0f}")

    # Save model
    artifacts_dir = Path(__file__).parent.parent / "artifacts"
    model_path = artifacts_dir / "lightgbm_selected_model.pkl"
    ...
    metrics_path = artifacts_dir / "lightgbm_selected_metrics.json"
    ...
    print("\nNext step: Train CatBoost with selected features")


if __name__ == "__main__":
    train_lightgbm_selected()
